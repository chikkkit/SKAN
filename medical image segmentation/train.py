...
from unikan.basis import lsin
from unikan.basis import lshifted_softplus
from unikan.basis import larctan
...
def parse_args():
    ...
    parser.add_argument('--model', default='KAN',
                        choices=['KAN', 'SKAN', 'MLP'],
                        help='model type: KAN | SKAN | MLP')
    parser.add_argument('--basis_function', default='lshifted_softplus',
                        choices=['lshifted_softplus', 'lsin', 'larctan', ''])
...
def wait_for_gpu(timeout=3600, interval=10):
    """
    wait for available GPU
    
    parameter:
        timeout: max waiting time(d), default 3600 s (1 hour)
        interval: interval of every detection of GPU(s), default 10 s
    """
    start_time = time.time()
    while True:
        # check if have available GPU
        if torch.cuda.is_available():
            print("CUDA GPU is available. Proceeding with the task.")
            break
        
        # calculate time consumed in total
        elapsed_time = time.time() - start_time
        if elapsed_time > timeout:
            print(f"Timeout reached ({timeout} seconds). No GPU is available.")
            exit(1)  # exit when timeout
        
        # output waiting message
        print(f"No GPU available. Retrying in {interval} seconds...")
        time.sleep(interval)
...
def main():
    ...
    elif config['model'] == 'SKAN':
        if config['basis_function'] == 'lshifted_softplus':
            basis = lshifted_softplus
        elif config['basis_function'] == 'lsin':
            basis = lsin
        elif config['basis_function'] == 'larctan':
            basis = larctan
        model = archs.USKAN(config['num_classes'],
                           config['input_channels'],
                           config['deep_supervision'],
                           embed_dims=config['input_list'],
                           basis=basis)
    ...
    train_transform = Compose([
        RandomRotate90(),
        Flip(),  # Replace geometric.transforms.Flip()
        Resize(config['input_h'], config['input_w']),
        Normalize(),  # Replace transforms.Normalize()
    ])
